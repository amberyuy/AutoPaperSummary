...
def create_history(history, model_name, metrics=None):
...
def save_plot_history(history, model_name, file_path, metrics=None):
    if metrics is None:
        metrics = {'acc', 'loss'}
    create_history(history, model_name, metrics)
...
def main():
    ...
    image_path = '/model/report' # you should change this path
    history_plot_path = image_path + '/' + seq_to_seq.model_name + '.png'
    if LOAD_EXISTING_WEIGHTS:
        history_plot_path = image_path + '/' + seq_to_seq.model_name + '-v' + str(summarizer.version) + '.png'
    
    save_plot_history(history, seq_to_seq.model_name, history_plot_path, metrics={'loss', 'acc'})
